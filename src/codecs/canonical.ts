...
function annotationRecord(value: unknown, path: string): AnnotationRegistryRecord {
    ...
    if (integrity === "resolved") {
        const orders = parts.map((part) => part.order).sort((a, b) => a - b);
        if (!orders.length || orders.some((order, index) => order !== index + 1)) {
            fail(`${path}.anchor.parts`, "resolved Markdown anchor requires contiguous part order 1..N");
        }
    }
    return {
        id: id(
...
export function validateRegistryPalette(registry: AnnotationRegistryV1, palette: PaletteCatalogueV1): void {
    const slots = new Set(palette.slots.map((slot) => slot.id));
    const concepts = new Set(palette.concepts.map((concept) => concept.id));
    for (const record of registry.records) {
        const snapshot = record.snapshot;
        if (
            snapshot?.paletteSlotId !== null &&
            snapshot?.paletteSlotId !== undefined &&
            !slots.has(snapshot.paletteSlotId)
        ) {
            fail(`annotation ${record.id}.snapshot.paletteSlotId`, `unknown PaletteSlotId ${snapshot.paletteSlotId}`);
        }
        if (
            snapshot?.semanticConceptId !== null &&
            snapshot?.semanticConceptId !== undefined &&
            !concepts.has(snapshot.semanticConceptId)
        ) {
            fail(
                `annotation ${record.id}.snapshot.semanticConceptId`,
                `unknown SemanticConceptId ${snapshot.semanticConceptId}`
            );
        }
    }
}
