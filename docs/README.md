...
B0 is pending. Slice 1 was accepted on 2026-09-25. Slice 2A's bounded first region (pure models/codecs/reconciliation and read-only desktop preparation) was completed on 2026-09-25; the backend and device checkpoint remains open. B0 requires the complete evidence set in [Contracts](CONTRACTS.md#b0-compatibility-baseline-gate) on desktop and real iPad/mobile, including substrate persistence and actual sync behavior. When accepted, record the build ID, schema versions, date, platforms, storage decisions, and evidence here and in `DECISIONS.md`. Do not create valuable FuturePlural annotations in a personal vault before explicit B0 acceptance and deployment authorization.
