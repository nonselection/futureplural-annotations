...
| Device-local active pointer/View/history | Obsidian Plugin.loadLocalStorage() / saveLocalStorage()           | Declarations mark the vault-specific helpers since 1.8.7. Restart retention, device/vault isolation, eviction, and iPad behavior remain untested.                                                                                                                                                                            |
...
- `Plugin.loadLocalStorage` / `saveLocalStorage` are present in local declarations with `@since 1.8.7` and described as vault-specific. Runtime availability, restart retention, and storage eviction are untested.
...
The Obsidian CLI is available on the host, but Codex's sandbox could not communicate with the running app. When desktop runtime evidence is needed, ask the user to run only the applicable bounded commands below in ordinary Terminal and return the output. The target vault is named `dev-vault`; the explicit `vault=` argument avoids relying on the active vault. Read/inspection commands:
