...
The existing plugin is still a pre-baseline Manager/Navigator implementation with positional highlight identity, legacy `data-fp-group` semantics, settings-array palette meanings, and numeric footnote labels. These are audited implementation facts, not the target architecture. Slice 0 changes documentation and records platform evidence only.
